    ...
    <div class="p-6" v-if="inv">
        ...
        <div class="flex flex-col md:flex-row justify-between items-center gap-4 mb-6">
            ...
            <div class="flex gap-2">
                ...
                <button @click="sendEmail" :disabled="loadingEmail || !inv.customer_id" title="Send invoice to customer email" class="flex items-center gap-2 bg-blue-600 hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed text-white px-4 py-2 rounded-lg font-semibold transition-colors shadow-sm">
                    <svg v-if="loadingEmail" class="animate-spin h-5 w-5 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
...
const sendEmail = async () => {
    if (!inv.value.customer_id) {
        alert('❌ Pelanggan belum diatur. Silakan perbarui invoice terlebih dahulu.');
        return;
    }

    if (!confirm('Kirim invoice via email ke pelanggan?')) return;

    loadingEmail.value = true;
    try {
        const response = await axios.post(`invoices/${route.params.id}/send-email`);
        alert('✅ ' + response.message);
        await fetchData();
    } catch (e) {
        const msg = e.response?.data?.message || 'Gagal mengirim email';
        alert('❌ ' + msg);
