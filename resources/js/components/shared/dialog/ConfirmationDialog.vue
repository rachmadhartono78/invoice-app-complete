<template>
  <!-- BACKDROP ANIMATION (Fades in/out separately) -->
  <Transition enter-active-class="transition-opacity duration-300 ease-out" enter-from-class="opacity-0"
    enter-to-class="opacity-100" leave-active-class="transition-opacity duration-200 ease-in"
    leave-from-class="opacity-100" leave-to-class="opacity-0">
    <!-- Backdrop must be highest priority -->
    <div v-if="isVisible" class="fixed inset-0 z-[60] bg-black bg-opacity-50"></div>
  </Transition>

  <!-- MODAL ANIMATION (Scales and fades separately) -->
  <Transition enter-active-class="transition-transform transition-opacity duration-300 ease-out"
    enter-from-class="opacity-0 scale-75" enter-to-class="opacity-100 scale-100"
    leave-active-class="transition-transform transition-opacity duration-200 ease-in"
    leave-from-class="opacity-100 scale-100" leave-to-class="opacity-0 scale-75">
    <div v-if="isVisible" class="fixed inset-0 z-[70] flex items-center justify-center">
      <div class="relative p-4 w-full max-w-md">
        <div class="relative bg-white rounded-xl shadow dark:bg-gray-700">
          <!-- Close Button -->
          <button type="button"
            class="absolute top-3 right-2.5 text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center dark:hover:bg-gray-600 dark:hover:text-white"
            @click="closeModal">
            <svg class="w-3 h-3" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 14 14">
              <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                d="M1 1l6 6m0 0l6 6M7 7l6-6M7 7L1 13" />
            </svg>
            <span class="sr-only">Close modal</span>
          </button>

          <!-- Modal Content -->
          <div class="p-4 md:p-5 text-center">
            <!-- Icon SLOT-->
            <slot name="icon">
              <svg class="mx-auto mb-4 text-gray-400 w-12 h-12 dark:text-gray-200" aria-hidden="true"
                xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 20 20">
                <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                  d="M10 11V6m0 8h.01M19 10a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z" />
              </svg>
            </slot>

            <!-- Message -->
            <h3 class="mb-5 text-lg font-normal text-gray-500 dark:text-gray-400">
              {{ message }}
            </h3>

            <!-- Action Buttons -->
            <button type="button" :disabled="loading"
              class="py-2.5 px-5 text-sm font-medium text-gray-900 focus:outline-none bg-white rounded-lg border border-gray-200 hover:bg-gray-100 hover:text-primary-700 focus:z-10 focus:ring-4 focus:ring-gray-100 dark:focus:ring-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:border-gray-600 dark:hover:text-white dark:hover:bg-gray-700"
              @click="closeModal">
              {{ cancelText }}
            </button>
            <button type="button" :disabled="loading"
              :class="['text-white', `bg-${confirmButtonColor}-600`, `hover:bg-${confirmButtonColor}-800`, `focus:ring-${confirmButtonColor}-300`, `dark:focus:ring-${confirmButtonColor}-800`, 'focus:ring-4', 'focus:outline-none', 'font-medium', 'rounded-lg', 'text-sm', 'inline-flex', 'items-center', 'px-5', 'py-2.5', 'text-center', 'ms-3']"
              @click="confirm">
              <svg aria-hidden="true" role="status" v-if="loading"
                class="inline w-4 h-4 me-3 text-gray-200 animate-spin dark:text-gray-600" viewBox="0 0 100 101"
                fill="none" xmlns="http://www.w3.org/2000/svg">
                <path
                  d="M100 50.5908C100 78.2051 77.6142 100.591 50 100.591C22.3858 100.591 0 78.2051 0 50.5908C0 22.9766 22.3858 0.59082 50 0.59082C77.6142 0.59082 100 22.9766 100 50.5908ZM9.08144 50.5908C9.08144 73.1895 27.4013 91.5094 50 91.5094C72.5987 91.5094 90.9186 73.1895 90.9186 50.5908C90.9186 27.9921 72.5987 9.67226 50 9.67226C27.4013 9.67226 9.08144 27.9921 9.08144 50.5908Z"
                  fill="currentColor" />
                <path
                  d="M93.9676 39.0409C96.393 38.4038 97.8624 35.9116 97.0079 33.5539C95.2932 28.8227 92.871 24.3692 89.8167 20.348C85.8452 15.1192 80.8826 10.7238 75.2124 7.41289C69.5422 4.10194 63.2754 1.94025 56.7698 1.05124C51.7666 0.367541 46.6976 0.446843 41.7345 1.27873C39.2613 1.69328 37.813 4.19778 38.4501 6.62326C39.0873 9.04874 41.5694 10.4717 44.0505 10.1071C47.8511 9.54855 51.7191 9.52689 55.5402 10.0491C60.8642 10.7766 65.9928 12.5457 70.6331 15.2552C75.2735 17.9648 79.3347 21.5619 82.5849 25.841C84.9175 28.9121 86.7997 32.2913 88.1811 35.8758C89.083 38.2158 91.5421 39.6781 93.9676 39.0409Z"
                  fill="#0f766e" />
              </svg>
              {{ loading ? 'Loading ..' : confirmText }}
            </button>
          </div>
        </div>
      </div>
    </div>
  </Transition>
</template>

<script>
export default {
  name: "ConfirmationDialog",
  props: {
    isVisible: {
      type: Boolean,
      required: true,
    },
    title: {
      type: String,
      default: "Konfirmasi aksi",
    },
    message: {
      type: String,
      default: "Apakah kamu yakin ingin melanjutkan?",
    },
    confirmButtonColor: {
      type: String,
      default: "primary",
    },
    confirmText: {
      type: String,
      default: "Ya, saya yakin",
    },
    cancelText: {
      type: String,
      default: "Tidak, batal",
    },
    loading: {
      type: Boolean,
      default: false,
    },
  },
  emits: ["onClose"],
  methods: {
    confirm() {
      this.$emit("onClose", true);
    },
    closeModal() {
      if (!this.loading) {
        this.$emit("onClose", false);
      }
    },
  },
};
</script>
