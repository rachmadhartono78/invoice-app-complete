<template>
    ...
    <div
        ...
        <div class="w-full mb-1">
            <div class="mb-4">
                <h1 class="text-xl font-semibold text-gray-900 sm:text-2xl dark:text-white">Kelola Menu</h1>
            </div>
            <table-molecule :columns="columns" :data="data" :loading="loading" @detail-clicked="handleDetail($event)"
                @edit-clicked="handleEdit($event)" @delete-clicked="confirmDelete($event)" showAddButton
                @onAdd="handleAdd()">
...
export default {
    ...
    methods: {
        // initModals() {
        //     const modalRefRead = this.$refs.modalRefRead;
        //     this.modalRead = new Modal(modalRefRead);
        // },
        doGetData() {
            this.loading = true;
        ...
        handleAdd() {
            this.$router.push({name:'create-menus'});
        },
        handleDetail(event) {
        ...
        handleEdit(event) {
            ...
            this.$router.push({ name: 'update-menus', params: { id: event.id } });
        },
        handleOnClose(event) {
            if (event === true) {
                console.log(event);
        ...
        submitDelete() {
            let url = "v1/settings/menus/" + this.selectedRow.id;
            this.loading = true;
            this.axios
                .delete(url)
                .then((res) => {
