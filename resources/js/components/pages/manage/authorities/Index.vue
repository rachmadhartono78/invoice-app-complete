<template>
    ...
    <div
        ...
        <div class="w-full mb-1">
            <div class="mb-4">
                <h1 class="text-xl font-semibold text-gray-900 sm:text-2xl dark:text-white">Kelola Otoritas</h1>
            </div>
...
export default {
    ...
    methods: {
        ...
        handleAdd() {
            this.$router.push({name: 'create-authorities'});
        },
        handleDetail(event) {
        ...
        handleEdit(event) {
            ...
            this.$router.push({ name: 'update-authorities', params: { id: event.id } });
        },
        handleOnClose(event) {
            if (event === true) {
                console.log(event);
        ...
        submitDelete() {
            let url = "v1/settings/authorities/" + this.selectedRow.id;
            this.loading = true;
            this.axios
                .delete(url)
                .then((res) => {
